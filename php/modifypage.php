...
<?php
    ...
    $sql = "SELECT * FROM member_tb WHERE email = '{$_SESSION['email']}'";
...
<body>
    ...
    <main role="main">
        <div class="container">
            <?php
                if(!isset($_SESSION['email'])){
                    echo '<a class="btn btn-outline-success my-2 my-sm-0" href="logout.php">먼저 로그인하십시오!</a>';
                } else {
            ?>
                <form method="post" action="">
                    <h3 class="text-center">Modify</h3>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="nickname" class="form-control" id="inputNickname" placeholder="Nickname" value="<?php echo $row['name']; ?>" required>
                        </div>
                    </div>
                    <div class="form-group">
                        <button type="submit" id="modify" class="btn btn-primary" onclick="$('#spinner').addClass('spinner-border spinner-border-sm');">
